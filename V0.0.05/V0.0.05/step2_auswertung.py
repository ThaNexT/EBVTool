...
def _get_session_dir_output() -> str:
    ...
    if _SESSION_TS_OUT is None:
        _SESSION_TS_OUT = datetime.now().strftime("%Y-%m-%d_%H-%M")
...
def _process_aggressivität(
    ...
    # Locate the company source workbook (must live alongside the tool).
    here = os.path.dirname(os.path.abspath(__file__))
    source_workbook = os.path.join(here, "2604XX_Rohdaten & Aggressivität.xlsx")
    if not os.path.exists(source_workbook):
        # Fallback name without space
        alt = os.path.join(here, "2604XX_Rohdaten_und_Aggressivität.xlsx")
        if os.path.exists(alt):
            source_workbook = alt
        else:
            print(
                f"  -> ERROR: Aggressivität source workbook not found at '{source_workbook}'. "
                "Place '2604XX_Rohdaten & Aggressivität.xlsx' next to the tool scripts."
            )
            return False
