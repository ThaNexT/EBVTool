...
def _get_session_dir_validation() -> str:
    ...
    if _SESSION_TS_VAL is None:
        _SESSION_TS_VAL = datetime.now().strftime("%Y-%m-%d_%H-%M")
...
def _ingest_ebv_dir(input_dir: str = INPUT_DIR_EBV) -> Tuple[List[Tuple[str, "pd.DataFrame"]], str]:
    ...
    for pdf_path in pdf_files:
        ...
        # Map "Wasser" → "undef" for Feststoff Soil_Type (Wasser is for Aggr).
        if soil_type.lower() == "wasser":
            soil_type = "undef"
        strat = bg_sample.get("Stratigraphie", "").strip()
        petro = bg_sample.get("Petrographische Beschreibung", "").strip()
...
def _ingest_pak_dir(input_dir: str = INPUT_DIR_PAK) -> Tuple[List[Tuple[str, "pd.DataFrame"]], str]:
    """Ingest all PAK lab PDFs. Same schema as EBV (reuses pdf_parser)."""
    return _ingest_ebv_dir(input_dir)
